...
class PostsIndexPage extends GetView<PostsIndexController> {
  ...
  Widget _buildView() {
    return EasyRefresh(
      ...
      child: CustomScrollView(
        slivers: [
          _buildPostCard(),
          Container(height: 2.h, color: Colors.grey[200]).paddingHorizontal(AppSpace.page.w).sliverToBoxAdapter(),
  ...
  Widget _buildPostCard() {
    ...
    return <Widget>[
          for (final i in list)
            <Widget>[TextWidget.body(i.key, weight: FontWeight.bold), TextWidget.muted(i.value)]
                ...
                .backgroundImage(DecorationImage(image: AssetImage(AssetsImages.imgPost_1Png), fit: BoxFit.cover))
                .onTap(() {
  ...
  Widget _buildHotTopic() {
    return GetBuilder<PostsIndexController>(
      ...
      builder: (_) {
        return <Widget>[
          TextWidget.body("热门话题", weight: FontWeight.bold).paddingOnly(left: AppSpace.page, top: 10.h, bottom: 10.h),
          ExpandableNotifier(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (index < 3) {
                    return ListTileWidget(
                      leading: IconWidget.svg("assets/svgs/ic_posts_hot${index + 1}.svg").paddingTop(4),
                      title: TextWidget.label(item.key, weight: FontWeight.bold),
                  ...
                  return ButtonWidget.text(
                    ...
                    icon: IconWidget.svg(AssetsSvgs.icArrowDownSvg, size: 16.r),
        ...
        ].toColumn(crossAxisAlignment: CrossAxisAlignment.start).sliverToBoxAdapter();
      },
    );
  }
  ...
  Widget _buildPostList() {
    return <Widget>[
      TextWidget.body("看看大家", weight: FontWeight.bold).alignment(Alignment.centerLeft).tight(height: 42.h),
    ...
    ].toColumn(crossAxisAlignment: CrossAxisAlignment.start).paddingHorizontal(16.w).sliverToBoxAdapter();
  }
  ...
  Widget _buildPostItem(Feed feed) {
    ...
    return <Widget>[
      ListTileWidget(
        backgroundColor: Colors.transparent,
        padding: EdgeInsets.zero,
        leading: ImageWidget.img("http://${feed.portrait}", width: 36.r, height: 36.r, radius: 50, fit: BoxFit.cover),
      ...
      GridView.count(
        ...
        children: List.generate(images.length, (index) {
          return ImageWidget.img(images[index], fit: BoxFit.cover, radius: 10).onTap(() {
    ...
    ].toColumn(crossAxisAlignment: CrossAxisAlignment.start).onTap(() => controller.toDetailPage(feed));
  }
      ...
      builder: (_) {
        return ScaffoldWidget(
          ...
          appBar: AppBarWidget(
            ...
            centerTitle: false,
            actions: [
              IconButton(onPressed: () {}, icon: IconWidget.svg(AssetsSvgs.icPostsSearchSvg)),
              IconButton(onPressed: () {}, icon: IconWidget.svg(AssetsSvgs.icPostsMoreSvg)),
            ],
          ),
          child: _buildView(),
        );
